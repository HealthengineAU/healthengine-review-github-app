...
const AUGMENT_PR_REQUEST_MARKER = /<!--\s*augment-pending\s*-->/;
...
export function isAugmentAuthored(user, body) {
  ...
  if (user.type === "Bot" && body && AUGMENT_PR_SUMMARY_MARKER.test(body) && !AUGMENT_PR_REQUEST_MARKER.test(body)) return true;
  return false;
}

// Wrap Copilot's "## Pull request overview" section in <details> so it starts
// collapsed. Boundary: from the heading to the end of the body. Newer Copilot
// reviews put the overview in their own <details> and skip this heading.
const COPILOT_OVERVIEW_HEADING = /^## Pull request overview\s*\n+/m;

// "Get a fresh assessment by requesting another Copilot review." — we re-review
// on our own terms, and the line sits above the useful content.
const COPILOT_FRESH_ASSESSMENT = /^[^\n]*requesting another Copilot review[^\n]*\n?/mi;

// Trailing "💡 Add a `code-review` agent skill ..." nudge and the `---` rule
// above it. We run several review providers, so the advice doesn't apply.
const COPILOT_SKILL_FOOTER =
  /\n+(?:(?:---|\*\*\*|___)[ \t]*\n+)?💡[^\n]*agent skill[^\n]*\n?/;
...
export function cleanCopilotBody(body) {
  ...
  let cleaned = body;
  if (COPILOT_FRESH_ASSESSMENT.test(cleaned) || COPILOT_SKILL_FOOTER.test(cleaned)) {
    cleaned = cleaned
      .replace(COPILOT_FRESH_ASSESSMENT, "")
      .replace(COPILOT_SKILL_FOOTER, "")
      .replace(EXTRA_BLANKS, "\n\n")
      .trimEnd();
  }

  const match = cleaned.match(COPILOT_OVERVIEW_HEADING);
  if (!match) return cleaned;
  const before = cleaned.slice(0, match.index);
  const rest = cleaned.slice(match.index + match[0].length).trimEnd();
...
// Wrap LinearB's "### ✨ PR Review" body in <details> so it starts collapsed,
// mirroring how we collapse Copilot and Auggie PR summaries. LinearB (powered
// by gitStream) posts these as pull_request_review bodies. Boundary: from the
// heading to the end of the body. The per-issue sections are already their own
// nested <details>, and GitHub renders nested details fine.
const LINEARB_REVIEW_HEADING = /^###\s*✨\s*PR Review\s*\n+/m;
const LINEARB_FOOTER_MARKER = /Generated by LinearB AI and added by gitStream/;

export function cleanLinearbBody(body) {
  if (!body) return body;
  const match = body.match(LINEARB_REVIEW_HEADING);
  if (!match) return body;
  const before = body.slice(0, match.index);
  const rest = body.slice(match.index + match[0].length).trimEnd();

  // Tiny PR case: LinearB only posts a short summary with no issue sections.
  if (!rest.includes("\n") && !/<br\s*\/?>/i.test(rest)) {
    return `${before}${rest}`;
  }

  // Typical case: summary + one or more issue <details> + gitStream footer.
  return `${before}<details>\n<summary><b>✨ LinearB PR Summary</b> <i>(click to expand)</i></summary>\n\n<br>\n\n${rest}\n\n</details>`;
}

export function isLinearbAuthored(user, body) {
  if (!user) return false;
  if (typeof user.login === "string" && /linearb/i.test(user.login)) return true;
  // Content-based fallback in case gitStream posts under a different login.
  if (user.type === "Bot" && body && LINEARB_FOOTER_MARKER.test(body)) return true;
  return false;
}
...
function cleanBody(user, body) {
  ...
  if (isLinearbAuthored(user, body)) return cleanLinearbBody(body);
  return body;
}
