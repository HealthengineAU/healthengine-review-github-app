...
const TRIGGER_PHRASES = /roast me|ai review|clanker review|clanker eyes/i;
...
const SUMMON_REPLY = `${AUGMENT_SUMMARY_MARKER}\n${AUGMENT_PENDING_MARKER}\n\n**{•<sup><sup>"</sup></sup>•} Auggie is being summoned...** \`augment review\`\n`;
...
async function notifyProviderDisabled(
  ...
  const currentProviderNames = [...config.providers].map(p => PROVIDER_DISPLAY_NAMES[p] ?? p);
  await octokit.rest.issues.createComment({
    owner,
    repo,
    issue_number,
    body: `**${name}** is not currently available. Active providers ([source](https://github.com/${owner}/.github/blob/main/.github/.github-bot-ai-reviewed-prs.yml)) are: ${currentProviderNames.join(", ") || "<none>"}`,
  });
}
...
export function register(app) {
  app.on(["issue_comment.created"], async (context) => {
    ...
    } else if (TRIGGER_PHRASES.test(body)) {
