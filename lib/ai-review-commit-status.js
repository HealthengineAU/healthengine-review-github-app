...
const BOT_COMMENTS = {
  [BOT.Augment]: {
    ...
    // [COMMENT_TYPE.Reviewed]: /Review completed\./,
  },

  [BOT.Copilot]: {
    [COMMENT_TYPE.Failed]: /Copilot wasn't able/,
    [COMMENT_TYPE.ApprovedNoFeedback]: /no comments|no new comments/,
    // [COMMENT_TYPE.Reviewed]: /Pull request overview|Copilot reviewed/,
  },
};
...
function classifyBot({ displayName, botKey, comments, feedbackTotal, feedbackResolved }) {
  ...
  if (detector?.[COMMENT_TYPE.Failed] && lastBody && detector[COMMENT_TYPE.Failed].test(lastBody)) {
    return { state: "failed", displayName };
  }

  if (detector?.[COMMENT_TYPE.ApprovedNoFeedback] && lastBody && detector[COMMENT_TYPE.ApprovedNoFeedback].test(lastBody)) {
    return { state: "approved-no-feedback", displayName };
  }

  if (feedbackTotal > 0 && feedbackResolved === feedbackTotal) {
    return { state: "approved", displayName, feedbackTotal };
  }

  return { state: "reviewed", displayName, feedbackTotal, feedbackResolved };
}
...
function formatBotEntry(entry) {
  ...
  if (entry.state === "approved-no-feedback") {
    return `Approved by ${entry.displayName} (No feedback)`;
  }

  if (entry.state === "approved") {
    const noun = entry.feedbackTotal === 1 ? "comment" : "comments";
    return `Approved by ${entry.displayName} (${entry.feedbackTotal} ${noun})`;
  }
  ...
  return `Reviewed by ${entry.displayName} (${entry.feedbackResolved}/${entry.feedbackTotal} resolved)`;
}
...
async function updateAIReviewStatus(octokit, { owner, repo, pr }) {
  ...
  for (const [userId, entry] of botActivity) {
    ...
    classifications.push({ ...result, login });
  }
  ...
  const pendingReviewClassifications = (pr.requested_reviewers || [])
    ...
    .map((user) => getBotDisplayName(user.login))
    .sort()
    .map((displayName) => ({ state: "requested-review", displayName }));
