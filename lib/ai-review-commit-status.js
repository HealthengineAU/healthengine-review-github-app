...
const COMMENT_TYPE = {
  Failed: "Failed",
  ReviewedNoFeedback: "ReviewedNoFeedback",
};

const BOT_COMMENTS = {
  [BOT.Augment]: {
    [COMMENT_TYPE.Failed]: /tokens to review|out of credits/,
    [COMMENT_TYPE.ReviewedNoFeedback]: /No suggestions at this time\./,
  },

  [BOT.Copilot]: {
    [COMMENT_TYPE.Failed]: /Copilot wasn't able|unable to review/,
    [COMMENT_TYPE.ReviewedNoFeedback]: /no comments|no new comments/,
  },
};
...
function classifyBot({
  ...
  if (
    detector?.[COMMENT_TYPE.ReviewedNoFeedback]
    && lastBody
    && detector[COMMENT_TYPE.ReviewedNoFeedback].test(lastBody)
  ) {
    return { ...data, state: "no-feedback" };
  }
  ...
  if (feedbackAcknowledged < feedbackTotal) {
    return { ...data, state: "unacknowledged" };
  }

  return { ...data, state: "acknowledged" };
}
...
function formatBotEntry(entry) {
  ...
  if (entry.state === "no-feedback") {
    return `✓ Reviewed by ${entry.displayName} (no feedback)`;
  }

  // prefer "resolved" when actually resolved
  const acknowledgeTerm = entry.feedbackResolved === entry.feedbackAcknowledged
    ? "resolved"
    : entry.feedbackResolved === 0
      ? "replied"
      : "resolved or replied";

  if (entry.state === "unacknowledged") {
    return `Reviewed by ${entry.displayName} (${entry.feedbackAcknowledged}/${entry.feedbackTotal} ${acknowledgeTerm})`;
  }

  return `✓ Reviewed by ${entry.displayName} (${entry.feedbackTotal} ${acknowledgeTerm})`;
}
...
async function updateAIReviewStatus(octokit, { owner, repo, pr }) {
  ...
  for (const [userId, entry] of botActivity) {
    ...
    classifications.push(classifyBot({
      ...
      feedbackWithReplies: threads.filter((t) => (t.comments?.totalCount || 0) > 1).length,
      feedbackAcknowledged: threads.filter((t) => t.isResolved || (t.comments?.totalCount || 0) > 1).length,
    }));
  }
  ...
  const pendingReviewClassifications = (pr.requested_reviewers || [])
    .filter((user) => isBotUser(user) && !classifiedUserIds.has(user.id))
    .map((user) => ({
