...
function formatBotEntry(entry) {
  ...
  if (entry.state === "failed") {
    return `${entry.displayName} was unable to review`;
  }

  if (entry.state === "approved") {
    const commentsNoun = entry.feedbackTotal === 1 ? "comment" : "comments";
    return `reviewed by ${entry.displayName} (${entry.feedbackTotal} resolved ${commentsNoun})`;
  }
  ...
  const unresolvedCount = Math.max(0, entry.feedbackTotal - entry.feedbackResolved);
  const commentsNoun = unresolvedCount === 1 ? "comment" : "comments";

  return `reviewed by ${entry.displayName} (${unresolvedCount} unresolved ${commentsNoun})`;
}
