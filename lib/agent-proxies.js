...
// An inline review comment. Only a mention counts, wherever the thread started
// and whoever opened it: a reply on the agent's own review thread is a human
// conversation, and the agent joins it when summoned, not by default. Humans
// only — bots post no-op noise and `ignore_users` covers service accounts that
// look like humans.
export function classifyReviewComment(agent, { commentAuthor, isBot, body }) {
  if (isBot) return null;
  const author = lc(commentAuthor);
  if (author === agent.botLower) return null;
  if (agent.ignoreUsers.has(author)) return null;
...
export function register(app) {
  ...
  app.on("pull_request_review_comment.created", async (context) => {
    ...
    const isBot = isBotUser(comment.user);
    for (const agent of agents) {
      const kind = classifyReviewComment(agent, {
        commentAuthor: comment.user?.login, isBot, body: comment.body,
      });
      if (!kind) continue;
      wakeAgent(context.octokit, agent, {
