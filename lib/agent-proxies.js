...
// An inline review comment. Unlike a PR-level comment, only an explicit mention
// counts — a diff thread between humans, even on the agent's own PR, is their
// conversation and not a summons.
export function classifyReviewComment(agent, { commentAuthor, isBot, body }) {
  if (isBot) return null;
  if (!agent.events.has("mention")) return null;
  const author = lc(commentAuthor);
  if (author === agent.botLower) return null;
  if (agent.ignoreUsers.has(author)) return null;
  if (!agent.mention?.test(body ?? "")) return null;
  return "mention";
}
