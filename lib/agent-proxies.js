...
// An inline review comment. A mention summons the agent anywhere; an unmentioned
// reply counts only on a thread the agent itself opened, where it is feedback on
// its own review. Other threads are other people's conversation.
export function classifyReviewComment(agent, { commentAuthor, isBot, body, threadStarter }) {
  if (!carriesFeedback(agent, { commentAuthor, isBot })) return null;
  if (agent.events.has("mention") && agent.mention?.test(body ?? "")) return "mention";
  if (agent.events.has("comment") && lc(threadStarter) === agent.botLower) return "comment";
  return null;
}

// An author whose inline comment could wake the agent at all, whatever thread it
// lands on. Bots post screenshots and other no-op noise, `ignore_users` covers
// service accounts that look human, and the agent never wakes on itself.
function carriesFeedback(agent, { commentAuthor, isBot }) {
  if (isBot) return false;
  const author = lc(commentAuthor);
  return author !== agent.botLower && !agent.ignoreUsers.has(author);
}

// Whether resolving the thread's opening comment could change the verdict — the
// one API call this handler makes, so it is worth not making blind.
export function needsThreadStarter(agent, { commentAuthor, isBot, body, inReplyToId }) {
  if (!inReplyToId) return false;
  if (!agent.events.has("comment")) return false;
  if (!carriesFeedback(agent, { commentAuthor, isBot })) return false;
  return classifyReviewComment(agent, { commentAuthor, isBot, body }) === null;
}
...
// Who opened the review thread a comment replies to. Fails soft to null: an
// unreadable comment must never break the delivery, it only means no wake.
async function fetchThreadStarter(octokit, { owner, repo, commentId }) {
  try {
    ...
    console.error(`[agent-proxies] thread lookup failed for ${owner}/${repo} comment ${commentId}:`, err.status ?? err.message);
    return null;
  }
}
...
export function register(app) {
  ...
  app.on("pull_request_review_comment.created", async (context) => {
    const { agents } = await loadAiReviewConfig(context);
    if (!agents.length) return;
    const { pull_request: pr, comment, repository } = context.payload;
    const { owner } = context.repo();
    const isBot = isBotUser(comment.user);
    const base = { commentAuthor: comment.user?.login, isBot, body: comment.body };
    // Resolved once per delivery, and only where a verdict turns on it. GitHub
    // points in_reply_to_id at the thread's root, not the previous reply.
    let starter;
    const threadStarter = async () => {
      if (starter === undefined) {
        starter = await fetchThreadStarter(context.octokit, {
          owner, repo: repository.name, commentId: comment.in_reply_to_id,
        });
      }
      return starter;
    };
    for (const agent of agents) {
      const kind = needsThreadStarter(agent, { ...base, inReplyToId: comment.in_reply_to_id })
        ? classifyReviewComment(agent, { ...base, threadStarter: await threadStarter() })
        : classifyReviewComment(agent, base);
      if (!kind) continue;
      wakeAgent(context.octokit, agent, {
        event: kind, owner, repo: repository.name, pr: pr.number,
        actor: comment.user?.login, body: comment.body,
        ackComment: comment.id, ackReviewComment: true,
      });
    }
  });
