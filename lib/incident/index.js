...
export function register(app, { getRouter } = {}) {
  ...
  // and the channel lookups ahead of views.open are capped well inside it.
  router.post("/slack/incident/command", rawBody, async (req, res) => {
    ...
    try {
      const [origin, incidents] = await Promise.all([
        lookupChannel({
          channelId: body.get("channel_id"),
          channelName: body.get("channel_name"),
          deps,
        }),
        lookupChannel({ channelId: channel, deps }),
      ]);
  ...
  router.post("/slack/incident/interact", rawBody, async (req, res) => {
    ...
    try {
      if (
        ...
        await openIncident({
          summary,
          destination: destinationFromView(payload.view),
          origin: viewOrigin(payload.view),
          user,
          responseUrl,
          deps,
        });
      ...
      if (payload.type === "block_actions") {
        ...
        // Both clear the ephemeral: Dismiss because that is all it does, and
        // View because Slack has already opened the thread in the browser and
        // the message has served its purpose. A url button still fires here.
        if (
          action?.action_id === "incident_view_thread" ||
          action?.action_id === "incident_dismiss"
        ) {
          if (responseUrl)
            await soft(() => postResponse(responseUrl, { delete_original: true }));
          return;
        }
        ...
        if (action.action_id === "incident_reopen")
        ...
        if (action.action_id === "incident_skip_report")
          return void (await dismissButtons({ source, deps }));
      }
    } catch (err) {
