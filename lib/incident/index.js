...
import { decodeRef, incidentModal, summaryFromView } from "./blocks.js";
import { markStatus, openIncident, requestReport, tellUser } from "./handlers.js";
...
export function register(app, { getRouter } = {}) {
  if (typeof getRouter !== "function") {
    app?.log?.warn?.("[incident] no router available - /slack/incident not mounted");
    return;
  }
  ...
  const jira = jiraClient({ baseUrl: jiraBaseUrl, email: jiraEmail, token: jiraToken });
  ...
  router.post("/slack/incident/command", rawBody, async (req, res) => {
    ...
    try {
      await deps.slack("views.open", { trigger_id: triggerId, view: incidentModal() });
    } catch (err) {
  ...
  router.post("/slack/incident/interact", rawBody, async (req, res) => {
    ...
    let payload;
    try {
      payload = JSON.parse(new URLSearchParams(req.rawBody ?? "").get("payload") ?? "{}");
    } catch {
      return;
    }
    ...
    try {
      if (payload.type === "view_submission" && payload.view?.callback_id === "incident_create") {
        const summary = summaryFromView(payload.view);
        if (!summary) return;
        await openIncident({ summary, user, deps });
        return;
      }
      ...
      if (payload.type === "block_actions") {
        ...
        if (action.action_id === "incident_mitigated") return void (await markStatus({ status: "mitigated", ref, user, deps }));
        if (action.action_id === "incident_resolved") return void (await markStatus({ status: "resolved", ref, user, deps }));
        if (action.action_id === "incident_draft_report") return void (await requestReport({ ref, user, deps }));
      }
    } catch (err) {
