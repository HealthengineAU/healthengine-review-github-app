...
export async function openIncident({ summary, user, responseUrl, deps }) {
  ...
  // The modal closes the instant we ack, and everything below is a dozen
  // sequential API calls. Say something now, and replace it with the result —
  // otherwise the form just vanishes and nothing appears for several seconds.
  if (responseUrl) {
    await soft(() =>
      postResponse(responseUrl, {
        text: ":hourglass_flowing_sand: Raising incident...",
      }),
    );
  }
...
export async function respond({ deps, responseUrl, user, text, blocks, replaceOriginal }) {
  if (responseUrl) {
    try {
