...
async function inviteIfMissing({ slack, channel, user }) {
  try {
    await slack("conversations.invite", { channel, users: user });
  } catch (err) {
    if (!err.message.endsWith(": already_in_channel")) throw err;
  }
}
...
export async function requestReport({ ref, source, user, deps }) {
  ...
  await once(`${key}:report`, async () => {
    await inviteIfMissing({ slack, channel, user: dustyUserId });
