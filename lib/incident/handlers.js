...
export async function openIncident({ summary, user, responseUrl, deps }) {
  ...
  await soft(() =>
    slack("reactions.add", { channel, timestamp: ts, name: ALERT_EMOJI }),
  );
  ...
  if (threadUrl) {
    await soft(() =>
      jira(`/rest/api/3/issue/${key}`, {
        ...
        body: {
          fields: {
            [JIRA.slack]: threadUrl,
            description: incidentDescription({ who, threadUrl }),
          },
        },
      }),
    );
...
export async function markStatus({ status, ref, user, deps }) {
  ...
  await soft(() =>
    slack("reactions.add", { channel, timestamp: thread, name: DONE_EMOJI }),
  );

  const stamp = jiraTimestamp();
  const fields = {
    [status === "resolved" ? JIRA.resolved : JIRA.mitigated]: stamp,
  };
  ...
  if (status === "resolved") {
    const current = await jira(
      `/rest/api/3/issue/${key}?fields=${JIRA.mitigated}`,
    );
