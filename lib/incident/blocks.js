...
export function incidentModal({ responseUrl, text, origin, incidents } = {}) {
  ...
  const destinations = [
    ...(origin?.postable && origin.id !== incidents?.id
      ? [option("current", `This channel (#${origin.name})`)]
      : []),
    option("incidents", `#${incidents?.name ?? "incidents"}`),
    option(
      "dedicated",
      "New private channel",
      "Code-named in Slack and Jira, e.g. incy-1024-wintery-snowfall",
    ),
  ];
    ...
    blocks: [
      {
        ...
        hint: {
          type: "plain_text",
          text: "5-10 words. Details can come later.",
        },
        element: {
      ...
      },
      {
        ...
        label: { type: "plain_text", text: "Where should it run?" },
        element: {
          type: "radio_buttons",
          action_id: "value",
        ...
        ? [
            {
              ...
              elements: [
                {
                  type: "mrkdwn",
                  text: "To run it in this channel, add this app to it first.",
                },
              ],
            },
          ]
