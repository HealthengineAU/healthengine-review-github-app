...
export function threadBlocks({ key, url, reporter, ref }) {
  const text = [
    ...
    "",
    "Mark as *mitigated* once ready to update the status of the incident.",
  ].join("\n");
...
function statusButtons(status, ref) {
  ...
  if (status === "mitigated") {
    return [
      ...
      button("incident_reopen", "Revert to open", "danger"),
    ];
  }
  if (status === "resolved") {
    return [button("incident_draft_report", "@Dusty - Draft incident report", "primary")];
  }
