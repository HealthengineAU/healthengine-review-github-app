...
export function threadBlocks({ key, url, reporter, ref }) {
  const text = [
    ...
    "",
    "Mark as *mitigated* or *resolved* when ready to update the status of the incident.",
  ].join("\n");
  ...
  return [
    { type: "section", text: { type: "mrkdwn", text } },
    {
      ...
      elements: [
        {
          type: "button",
          action_id: "incident_mitigated",
          text: { type: "plain_text", text: "Mark as mitigated" },
          value: ref,
        },
        {
          type: "button",
...
export function statusThreadBlocks({ key, url, status, who, ref }) {
  ...
  return [
    { type: "section", text: { type: "mrkdwn", text } },
    {
      ...
      elements: [
        // Pressing either status button removes both, so a mitigated incident
        // carries its Resolve button forward on this message.
        ...(status === "mitigated"
          ? [
              {
                type: "button",
