...
// The config gate: kill switch, target-branch allowlist, repo/author
// exclusions, and diff-size bounds (inclusive, additions + deletions).
export function isAutoReviewEligible({ autoReview, repo, pr }) {
  ...
  if (!autoReview.includeBranches.has(baseBranch)) return false;
