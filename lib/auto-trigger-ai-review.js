...
// Anything already reviewed or in flight (a requested Copilot, an Auggie or
// Dusty summon, an AI-review team request, a gitStream/LinearB run) means no
// automatic invite.
async function evaluateAndSummon(context, { owner, repo, pr, config }) {
  ...
  await triggerRandomReviewer(context.octokit, {
    ...
    // Nobody asked for this one, so the PR's own author is who it's charged to.
    actor: pr.user?.login,
  });
}
