...
// Dusty is summoned by tagging its team in the org that owns the repo.
export const dustySummonComment = (owner) => `@${owner}/dusty review`;
...
// Dusty's agent proxy, when one is configured with the `mention` event. Without
// it nothing can reach Dusty, so it isn't summonable however `providers` reads
// — the no-config default included.
function dustyAgent(config) {
  const agent = (config?.agents ?? []).find((a) => a.name.toLowerCase() === BOT.Dusty);
  return agent?.events.has("mention") ? agent : null;
}
...
export async function triggerDustyReviewer(octokit, { owner, repo, issue_number, config, actor }) {
  const agent = dustyAgent(config);
  if (!agent) return;

  const body = dustySummonComment(owner);

  const summon = await octokit.rest.issues.createComment({ owner, repo, issue_number, body });
...
const AI_REVIEWER_TRIGGERS = [
  ...
  { provider: BOT.Dusty, trigger: triggerDustyReviewer, reachable: dustyAgent },
];

// Enabled in `providers` AND actually reachable from this repo's config.
export function isProviderSummonable(config, provider) {
  const entry = AI_REVIEWER_TRIGGERS.find((t) => t.provider === provider);
  if (!entry || !config.isProviderEnabled(provider)) return false;
  return entry.reachable ? entry.reachable(config) != null : true;
}

// True when at least one summonable reviewer (Auggie, Copilot, Dusty) is
// available — lets callers skip work that would end in a triggerRandomReviewer
// no-op.
export function canSummonReviewer(config) {
  return AI_REVIEWER_TRIGGERS.some((t) => isProviderSummonable(config, t.provider));
}
...
export async function triggerRandomReviewer(octokit, { owner, repo, issue_number, config, diffSize, linesAdded, actor }) {
  const enabled = AI_REVIEWER_TRIGGERS.filter((t) => isProviderSummonable(config, t.provider));
  if (enabled.length === 0) {
    return;
  }
...
// "@<org>/dusty review", optionally backticked). The Dusty form must carry the
// "@": that is what the agent proxy's mention matcher wakes on, and an
// unqualified "dusty review" would read as pending forever. Exact-body on
...
const SUMMON_COMMANDS = {
  ...
  [BOT.Dusty]: /^\s*`*\s*@(?:[\w-]+\/)?dusty review\s*`*\s*$/i,
};
