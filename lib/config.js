...
function normalizeLogins(raw) {
  ...
  if (!Array.isArray(raw)) return logins;

  for (const entry of raw) {
    if (typeof entry !== "string") continue;
    const login = entry.trim().replace(/^@/, "").toLowerCase();
...
export async function loadAiReviewConfig(context) {
  ...
  // Allow-list of GitHub logins permitted to trigger privileged actions such as
  // the mergebot's `admin merge`. There is no default — an absent/invalid list
  // means nobody is authorized (fail closed). This is only read from the repo's
  // default branch / org `.github` repo (never a PR branch), so it can't be
  // escalated by editing the file inside a pull request.
  const mergeAdmins = normalizeLogins(raw?.merge_admins);
  ...
  const config = {
    ...
    isMergeAdmin(login) {
      return (
        typeof login === "string" &&
        mergeAdmins.has(login.trim().replace(/^@/, "").toLowerCase())
      );
    },
  };
