...
// exclusions, only PRs targeting a mainline branch, up to 2000 changed lines.
const AUTO_REVIEW_DEFAULTS = {
  includeBranches: ["master", "main", "develop"],
  minDiffSize: 0,
  maxDiffSize: 2000,
};
...
export function normalizeAutoReview(raw) {
  ...
  // Unlike the exclusion lists, an empty/invalid include list falls back to
  // the defaults — auto review needs at least one eligible target branch.
  const includeBranches = toNameSet(raw?.include_branches);

  return {
    enabled: raw?.enabled !== false,
    includeBranches: includeBranches.size > 0
      ? includeBranches
      : new Set(AUTO_REVIEW_DEFAULTS.includeBranches),
