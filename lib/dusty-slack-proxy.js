...
// The acknowledgement reaction — added seconds after the tag, rather than the ~30s-2min a
// runner takes to boot, so it means "received and queued", nothing more. Dusty's
// mark-progress.sh clears it and raises the "is working…" bar once a leg is running.
const ACK_EMOJI = "dusty_is_on_it";
...
// Slack app_ids/bot_ids whose messages are allowed to tag Dusty, from
// SLACK_ALLOWED_BOT_APPS. Must never include Dusty's own app — that would let
// Dusty wake itself.
export function parseAllowedBots(raw) {
  return new Set(
    String(raw ?? "")
      .split(",")
      .map((s) => s.trim())
...
// webhook_event.yml needs. Returns null to ignore (our own messages, unlisted
// bots, wrong workspace, malformed). thread_ts||ts as the key means a mid-thread
// tag adopts the whole thread.
export function classifyMention(event, { teamId, allowedTeam, allowedBots, selfUserId } = {}) {
  if (!event || event.type !== "app_mention") return null;
  if (allowedTeam && teamId && teamId !== allowedTeam) return null;

  const bot = event.bot_id || event.app_id;
  if (bot && !(allowedBots?.has(event.app_id) || allowedBots?.has(event.bot_id))) return null;

  const channel = event.channel;
  const thread = event.thread_ts || event.ts;
  const rawText = String(event.text ?? "");

  // A bot is never its own requester: a workflow's message carries no `user` at
  // all, and an app posts as its own bot user. Either way the requester is whoever
  // the message tags alongside Dusty; without our own id we cannot tell them apart.
  let actor = event.user;
  if (bot) {
    if (!selfUserId) return null;
    actor = [...rawText.matchAll(MENTION)]
      .map((m) => m[1])
      .find((id) => id !== selfUserId && id !== event.user);
  }

  if (!channel || !thread || !actor) return null;
...
// reactions.add needs channel membership; posting and streaming never do. Join first and
// try the reaction regardless — `already_in_channel` and `not_in_channel` are both fine, as
// is `already_reacted` on a Slack retry that slipped past the event_id cache.
export async function ackReceived({ channel, ts, token, call = slackCall }) {
  ...
  try {
    ...
    await call("reactions.add", { channel, timestamp: ts, name: ACK_EMOJI }, token);
  } catch {
    // A missing acknowledgement is not news.
  }
}
...
export function register(app, { getRouter } = {}) {
  ...
  const allowedBots = parseAllowedBots(process.env.SLACK_ALLOWED_BOT_APPS);
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const inputs = classifyMention(payload.event, {
        teamId: payload.team_id,
        allowedTeam,
        allowedBots,
        selfUserId: payload.authorizations?.[0]?.user_id,
      });
      if (!inputs) return;
      ...
      try {
        ...
        // Only once the run is queued — an ack before a failed dispatch would claim work
        // nobody is doing. On the tagging message, not the thread root.
        await ackReceived({
