...
export function classifyMention(event, { teamId, allowedTeam, allowedBots, selfUserId } = {}) {
  ...
  // A workflow's message carries no `user`, so the requester is whoever the
  // workflow tagged alongside Dusty; without our own id we cannot tell them apart.
  let actor = event.user;
  if (!actor) {
    if (!bot || !selfUserId) return null;
    actor = [...rawText.matchAll(MENTION)].map((m) => m[1]).find((id) => id !== selfUserId);
  }
