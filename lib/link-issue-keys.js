...
// A KEY-123 token: the key starts on a non-alphanumeric boundary, and the
// number isn't followed by a digit or a version-style ".4", so
// "core-app-1.2.328" and "joi-17.13.8" aren't issues. A trailing letter is a
// sub-branch marker ("ABC-284a") and isn't part of the reference.
...
// appear. The first rule whose keys match wins.
export function extractReferences(text, rules) {
  const references = [];
  if (typeof text !== "string") return references;

  for (const [, key, number] of text.matchAll(REFERENCE)) {
    const rule = rules.find(({ keys }) => matchesFilterPatterns(keys, key));
    if (!rule) continue;
...
export function withIssueLinks({ body, references }) {
  ...
  const existing = typeof body === "string" ? body.replace(/^\s+/, "") : "";
...
export function register(app) {
  app.on("pull_request.opened", async (context) => {
    ...
    await context.octokit.rest.pulls.update({
      ...
      body: withIssueLinks({ body: pr.body, references }),
    });
  });
}
